...
fun GestionLogrosView(
    ...
    val logros by adminViewModel.logrosGenerales.collectAsState()

    val opcionesMenu = listOf(
        OpcionMenu("Usuarios", Icons.Default.Group, 0),
        OpcionMenu("Logros", Icons.Default.EmojiEvents, 1),
        OpcionMenu("Tareas", Icons.Default.Task, 2),
        OpcionMenu("Cerrar sesión", Icons.Default.ExitToApp, 3)
    )

    var opcionSeleccionada by remember { mutableStateOf(opcionesMenu[1]) }
        ...
        Scaffold(
            ...
            floatingActionButton = {
                ExtendedFloatingActionButton(
                    text = { Text("Añadir Logro") },
                    icon = { Icon(Icons.Default.Add, null) },
                    onClick = {
                        navHostController.navigate(Rutas.NuevoLogro)
                    },
                    containerColor = DarkBackground,
            ...
            Column(
                ...
                LaunchedEffect(Unit) {
                    adminViewModel.cargarLogrosGenerales()
                }
